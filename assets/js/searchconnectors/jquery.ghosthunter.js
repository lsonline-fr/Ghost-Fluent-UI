 ...
 */
  ...
  var grabAndIndex = function () {
    ...
    var ready = { "posts": false, "pages": false };
    var ghost_root;
    try {
    ...
    $.get(postsUrl).done(function (data) {
      ...
      idxSrc = idxSrc.concat(postsList);
      ready.posts = true;
      if (ready.posts && ready.pages) {
        grabAndIndexAll(idxSrc, me);
      }
    });

    var pagesUrl = ghost_root + "/content/pages/?key=" + ghosthunter_key + "&limit=all&include=tags";

    $.get(pagesUrl).done(function (data) {
      var pagesList = data.pages;
      pagesList.forEach(function (p) {
        p.type = "page";
      });
      idxSrc = idxSrc.concat(pagesList);
      ready.pages = true;
      if (ready.posts && ready.pages) {
        grabAndIndexAll(idxSrc, me);
      }
  ...
  var grabAndIndexAll = function (idxSrc, ctx) {
    //console.log('ghostHunter: grabAndIndexAll', idxSrc);
    ctx.blogData = {};
    ctx.latestPost = 0;
    ...
    me.index = lunr(function () {
      ...
      idxSrc.forEach(function (arrayItem) {
        ...
        me.blogData[arrayItem.id] = {
          ...
          description: arrayItem.custom_excerpt ? arrayItem.custom_excerpt : '',
  ...
  var pluginMethods = {
    ...
    loadAPI: function () {
      ...
      if (this.isInit) {
        ...
        var pagesUrl = ghost_root + "/content/pages/?key=" + ghosthunter_key + "&limit=all&fields=id" + "&filter=" + "updated_at:>\'" + this.latestPost.replace(/\..*/, "").replace(/T/, " ") + "\'";
